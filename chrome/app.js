    ...
    this.model.event.addListener('onSubmitAttachments', function(target, params) {
        ...
        if (attach1) {
            if (!attach1.hasOwnProperty('checked') || attach1.checked !== true) { // Skip this attachment
                ...
                self.urlToData(attach1, function(trello_attach) {
 ...
 * From: https://miguelmota.com/bytes/xmlhttprequest-multipart-post
 */
GmailToTrello.App.prototype.encodeForTransfer = function(mimeType, name, arrayBuffer) { 
    const dataView = new DataView(arrayBuffer);

    const BOUNDARY = 'BOUNDARY1234';
    const BOUNDARY_DASHES = '--';
    const NEWLINE = '\r\n';
    const ACTUAL_CONTENT_TYPE = 'Content-Type: ' + mimeType;
    const ACTUAL_CONTENT_DISPOSITION = 'Content-Disposition: form-data; name="' + name + '"';
 ...
 */
GmailToTrello.App.prototype.urlToData = function(attach1, callback) {
    var self = this;
    ...
    if (attach1 && attach1.url.length > 0) {
        ...
        xhr.onload = function() {
            ...
            fileReader.onload = function() {
                ...
                var data = self.encodeForTransfer(mimeType, name, result);
                callback({
